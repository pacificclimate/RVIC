...
class Tape(object):
    ...
    def __init__(
        ...
        if outtype.lower() == "grid":
            ...
            if isinstance(grid_lons, check_types) and isinstance(
                grid_lats, check_types
            ):
        ...
        if units in [
            ...
            self._units_mult = np.ones_like(self._outlet_y_ind, dtype=np.float64)
        elif units in ["m3/s", "m^3/s", "m3 s-1"]:
            # kg/m2/s --> m3/s
            self._units_mult = grid_area[self._outlet_y_ind, self._outlet_x_ind]
        ...
        for field in self._fincl:
            self._temp_data[field] = np.zeros(self._num_outlets, dtype=np.float64)
        # ------------------------------------------------------------ #
    ...
    def __repr__(self):
        parts = [
            "------- Summary of History Tape Settings -------",
    ...
    def update(self, data2tape, time_ord):
        ...
        if self._time_ord != time_ord:
            raise ValueError(
                "rout_var.time_ord does not match the time_ord "
        ...
        self._timestamp = ord_to_datetime(
            self._time_ord, TIMEUNITS, calendar=self._calendar
        )
        # ------------------------------------------------------------ #
        ...
        for field in self._fincl:
            ...
            if self._avgflag == "A":
                ...
                raise ValueError(
                    "Average flag ({0}) does not match any of"
        ...
        if self._mfilt == "year":
            ...
            if self._nhtfrq == 0:
                ...
                if self._timestamp.month == 12:
                    ...
                    t1 = datetime(self._timestamp.year, self._timestamp.month + 1, 1)
                b1 = date2num(t1, TIMEUNITS, calendar=self._calendar)
    ...
    def __update_out_data(self):
        ...
        for field in self._fincl:
            if self._outtype == "grid":
                # ---------------------------------------------------- #
                # Grid the fields
                self._out_data[field][
                    self._out_data_i, self._outlet_y_ind, self._outlet_x_ind
            ...
            if np.isnan(self._out_data[field][self._out_data_i].sum()):
                raise ValueError(
                    "nan found in output field: {0}, most likely "
        ...
        if self._out_data_i == self._out_data_write:
            ...
            self._out_data_i += 1
            log.debug(
                "out_data counter is %s of %s", self._out_data_i, self._out_data_write
            )
    ...
    def __next_update_out_data(self):
        ...
        if self._nhtfrq == 0:
            if self._timestamp.month == 12:
                b1 = date2num(
                    datetime(self._timestamp.year + 1, 2, 1),
                    TIMEUNITS,
                    calendar=self._calendar,
                )
            else:
                b1 = date2num(
                    datetime(self._timestamp.year, self._timestamp.month + 1, 1),
                    TIMEUNITS,
                    calendar=self._calendar,
                )
        ...
        if self._avgflag == "I":
            self._write_ord = b1
            self.filename = num2date(b1, TIMEUNITS, calendar=self._calendar).strftime(
                self._fname_format
            )
        else:
            self._time_bnds = np.array([[b0, b1]])
            self._write_ord = np.average(self._time_bnds)
            self.filename = num2date(b0, TIMEUNITS, calendar=self._calendar).strftime(
                self._fname_format
            )
        self.rest_filename = num2date(b1, TIMEUNITS, calendar=self._calendar).strftime(
            self._fname_format
        )
        # ------------------------------------------------------------ #
    ...
    def __write_grid(self):
        ...
        if self._avgflag != "I":
            ...
            time_bnds = f.createVariable(
                "time_bnds", self._ncprec, ("time", "nv",), **self.ncvaropts
            )
        ...
        if self._grid_lons.ndim > 1:
            ...
            lon = f.createVariable("lon", self._ncprec, ("lon",), **self.ncvaropts)
            lat = f.createVariable("lat", self._ncprec, ("lat",), **self.ncvaropts)
            lon[:] = self._grid_lons
            lat[:] = self._grid_lats
        ...
        for field in self._fincl:
            var = f.createVariable(field, self._ncprec, tcoords, **self.ncvaropts)
    ...
    def __write_array(self):
        ...
        time = f.createVariable("time", self._ncprec, ("time",), **self.ncvaropts)
        time[:] = self._out_times[: self._out_data_i]
        for key, val in iteritems(share.time):
            if val:
                setattr(time, key, val.encode())
        ...
        # ------------------------------------------------------------ #
        # Variables
        outlet_lon = f.createVariable("lon", self._ncprec, coords, **self.ncvaropts)
        outlet_lat = f.createVariable("lat", self._ncprec, coords, **self.ncvaropts)
        outlet_x_ind = f.createVariable(
            "outlet_x_ind", NC_INT, coords, **self.ncvaropts
        )
        outlet_y_ind = f.createVariable(
            "outlet_y_ind", NC_INT, coords, **self.ncvaropts
        )
        outlet_decomp_ind = f.createVariable(
            "outlet_decomp_ind", NC_INT, coords, **self.ncvaropts
        )
        onm = f.createVariable("outlet_name", NC_CHAR, nocoords, **self.ncvaropts)
