    ...
    for i, (y, x) in enumerate(pyzip(routys, routxs)):
        ...
        for j in range(search + 1):
            ...
            if np.any(search_area / area0 > tol):
                ...
                log.debug(
                    "Moving pour point to channel y: %s->%s, x: %s->%s",
                    y,
                    new_ys[i],
                    x,
                    new_xs[i],
                ...
                log.debug(
                    "Source Area has increased from %s to %s",
                    area0,
                    source_area[new_ys[i], new_xs[i]],
                )
    ...
    for key, value in iteritems(config_section):
        if key.endswith("var"):
            if value not in nckeys:
                ...
                raise NameError(
                    "Check netcdf that netcdf variable names match"
    ...
    for key, section in iteritems(config_dict):
        if "FILE_NAME" in list(section.keys()):
            ...
            config.set(
                key,
                "FILE_NAME",
                os.path.join(inputs_dir, os.path.split(section["FILE_NAME"])[1]),
            )
...
def tar_inputs(inputs, suffix="", tar_type="tar"):
    ...
    if tar_type == "tar":
        ...
        log.warning("Unknown tar_type: %s, proceeding with gunzipped mode", tar_type)
        end = ".tgz"
        mode = "w:"
...
def read_domain(domain_dict, lat0_is_min=False):
    ...
    dom_data["cell_ids"] = np.arange(dom_data[dom_mask].size).reshape(
    ...
    if dom_data[dom_lon].ndim == 1:
        ...
        if (dom_data[dom_lat][-1] > dom_data[dom_lat][0]) != lat0_is_min:
            log.debug("Domain Inputs came in upside down, flipping everything " "now.")
        ...
        dom_data[dom_lon], dom_data[dom_lat] = np.meshgrid(
            dom_data[dom_lon], dom_data[dom_lat]
        )
        # ------------------------------------------------------------- #
    # ---------------------------------------------------------------- #
    ...
    if area_units in [
        ...
        log.warning(
            "WARNING: UNKNOWN AREA units (%s), ASSUMING THEY ARE IN " "SQUARE METERS",
