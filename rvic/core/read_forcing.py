...
class DataModel(object):
    ...
    # ---------------------------------------------------------------- #
    # Initialize
    def __init__(self, path, file_str, time_fld, lat_fld, liq_flds, start, end):
        ...
        # yearly files
            ...
            for year in pyrange(start[0], end[0] + 1):
                self.files.append(
                    os.path.join(
                        self.path, file_str.replace("$YYYY", "{0:04d}".format(year))
                    )
                )
        ...
        # Monthly
            ...
            while True:
                self.files.append(
                    os.path.join(
                        self.path,
                        file_str.replace("$YYYY", "{0:04d}".format(year)).replace(
            ...
            while True:
                self.files.append(
                    os.path.join(
                        ...
                        .replace("$MM", "{0:02d}".format(month))
                        .replace("$DD", "{0:02d}".format(day)),
                    )
                )
        ...
        for i, fname in enumerate(self.files):
            ...
            if i == 0:
                ...
                if lats.ndim == 1:
                    if lats[-1] > lats[0]:
                        log.debug(
                            "Input fluxes came in upside down, flipping "
        ...
        self.end = ord_to_datetime(
            self.end_ords[-1], self.time_units, calendar=self.calendar
        )
        # ------------------------------------------------------------ #
        ...
        if len(time_series) > 1:
            t0 = date2num(
                num2date(time_series[0], self.time_units, calendar=self.calendar),
                TIMEUNITS,
                calendar=self.calendar,
            )
            t1 = date2num(
                num2date(time_series[1], self.time_units, calendar=self.calendar),
                TIMEUNITS,
                calendar=self.calendar,
            )
            ...
            raise ValueError(
                "Need more than 1 forcing timestep in order to " "calculate timestep"
            )
        # ------------------------------------------------------------ #
    ...
    def start(self, timestamp, rout_var):
        ...
        try:
            ...
            log.error(
                num2date(self.start_dates, self.time_units, calendar=self.calendar)
            )
        ...
        for fld in self.liq_flds:
            ...
            try:
                fill_val = getattr(self.current_fhdl.variables[fld], "_FillValue")
                ...
                if np.any(fmask[rout_var.source_y_ind, rout_var.source_x_ind]):
                    ...
                    raise ValueError(
                        "Exiting now due to fill values being "
    ...
    def read(self, timestamp):
        ...
        for flux, fields in (("LIQ", self.liq_flds), ("ICE", self.ice_flds)):
            for i, fld in enumerate(fields):
                ...
                fill_val = getattr(self.current_fhdl.variables[fld], "_FillValue", None)
