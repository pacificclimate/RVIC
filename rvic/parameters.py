...
def gen_uh_init(config):
    ...
    if config_dict["OPTIONS"]["REMAP"] and not remap_available:
        raise ValueError(
            "Problem importing remap module " "check to make sure cdo.py is available)"
        )
    # ---------------------------------------------------------------- #
    ...
    directories.update(
        make_directories(config_dict["OPTIONS"]["TEMP_DIR"], ["aggregated", "remapped"])
    )
    # ---------------------------------------------------------------- #
    ...
    try:
        ...
        if not (
            all(x in list(pour_points.keys()) for x in ["lons", "lats"])
            or all(x in list(pour_points.keys()) for x in ["x", "y"])
        ):
            raise ValueError(
                "Pour Points File must include " "variables (lons, lats) or (x, y)"
            )
        if "names" in pour_points:
            pour_points.fillna(inplace=True, value="unknown")
            for name in pour_points.names:
                pour_points.loc[
                    pour_points["names"] == name, "names"
                ] = strip_invalid_char(name)

        pour_points.drop_duplicates(inplace=True)
        pour_points.dropna()
    except Exception as e:
        log.error(
            "Error opening pour points file: %s",
    ...
    uh_box = {}
    try:
        uh_box["time"], uh_box["func"] = np.genfromtxt(
            uh_file, skip_header=uh_header, delimiter=",", unpack=True
        )
        log.info("Opened UHbox File: %s", config_dict["UH_BOX"]["FILE_NAME"])
    except:
        log.exception(
            "Error opening uh_box file: %s", config_dict["POUR_POINTS"]["FILE_NAME"]
        )
        raise
    # ---------------------------------------------------------------- #
    ...
    try:
        ...
        if fdr_data[fdr_lat][-1] > fdr_data[fdr_lat][0]:
            log.debug(
                "Flow Direction inputs came in upside down, flipping " "everything now."
            )
        ...
        if not isinstance(fdr_vel, basestring):
            fdr_data["velocity"] = np.zeros(fdr_shape, dtype=np.float64) + fdr_vel
            config_dict["ROUTING"]["VELOCITY"] = "velocity"
            log.info("Added velocity grid to fdr_data")
        if not isinstance(fdr_dif, basestring):
            fdr_data["diffusion"] = np.zeros(fdr_shape, dtype=np.float64) + fdr_dif
            config_dict["ROUTING"]["DIFFUSION"] = "diffusion"
            log.info("Added diffusion grid to fdr_data")
        if (
            "SOURCE_AREA_VAR" not in config_dict["ROUTING"]
            or config_dict["ROUTING"]["SOURCE_AREA_VAR"] not in fdr_data
        ):
            log.warning(
                "Upstream `SOURCE_AREA` was not provided, output "
                "source area will be zero."
            )
            config_dict["ROUTING"]["SOURCE_AREA_VAR"] = "src_area"
            fdr_data[config_dict["ROUTING"]["SOURCE_AREA_VAR"]] = np.zeros(
                fdr_shape, dtype=np.float64
            )
        # ---------------------------------------------------------------- #

        # ---------------------------------------------------------------- #
        fdr_data["resolution"] = np.abs(fdr_data[fdr_lon][1] - fdr_data[fdr_lon][0])
    ...
    if options["AGGREGATE"] and not options["REMAP"]:
        log.error(
            "RVIC parameter generation requires REMAP option to be True"
    ...
    if not options["REMAP"]:
        domain_res = np.abs(
            dom_data[domain["LONGITUDE_VAR"]][0, 1]
        ...
        if not np.isclose(fdr_data["resolution"], domain_res):
            ...
            raise ValueError(
                "If remap is false, domain and routing grid " "resolutions must match."
            )
    # ---------------------------------------------------------------- #
    ...
    if options["AGGREGATE"]:
        ...
        outlets = OrderedDict()
        if all(x in list(pour_points.keys()) for x in ["x", "y", "lons", "lats"]):
            ...
            routys, routxs = latlon2yx(
                plats=lats, plons=lons, glats=fdr_data[fdr_lat], glons=fdr_data[fdr_lon]
            )
...
def gen_uh_run(uh_box, fdr_data, fdr_vatts, dom_data, outlet, config_dict, directories):
    ...
    for j, pour_point in enumerate(outlet.pour_points):
        ...
        rout_data = rout(
            pour_point, uh_box, fdr_data, fdr_vatts, config_dict["ROUTING"]
        )

        log.debug("Done routing to pour_point")
        log.debug(
            "rout_data: %s, %s",
            rout_data["unit_hydrograph"].min(),
            rout_data["unit_hydrograph"].max(),
        )
        log.debug(
            "rout_data sum: %s, %s",
            rout_data["unit_hydrograph"].sum(),
    ...
    if options["REMAP"]:
        glob_atts = NcGlobals(
            ...
            RvicPourPointsFile=os.path.split(config_dict["POUR_POINTS"]["FILE_NAME"])[
                1
            ],
        ...
        write_agg_netcdf(
            temp_file_1, agg_data, glob_atts, options["NETCDF_FORMAT"], **ncvaropts
        )
        ...
        if final_data[dom_lat].ndim == 1:
            if final_data[dom_lat][-1] > final_data[dom_lat][0]:
                ...
                log.debug(
                    "Remapped inputs came in upside down, flipping %s",
                    ", ".join(var_list),
                )
                ...
                final_data["unit_hydrograph"] = final_data["unit_hydrograph"][
                    :, ::-1, :
                ]
        ...
        uh_shape = (agg_data["unit_hydrograph"].shape[0],) + dom_data[dom_mask].shape
        final_data = {}
        final_data["unit_hydrograph"] = np.zeros(uh_shape, dtype=np.float64)
        final_data["fraction"] = np.zeros(dom_data[dom_mask].shape, dtype=np.float64)
        ...
        if (ys < 0).any() or (xs < 0).any():
            raise ValueError(
                "Negative indicies found when mapping "
                "`non-remapped` rout_data to domain grid."
            )

        final_data["unit_hydrograph"][:, ys, xs] = agg_data["unit_hydrograph"][
            :, bys, bxs
        ]
