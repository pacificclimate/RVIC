...
def run_examples(config_file):
    ...
    for i, (test, test_dict) in enumerate(iteritems(config_dict)):
        print("".center(100, "-"))
        print(
            "Starting Test #{0} of {1}: {2}".format(i + 1, num_tests, test).center(100)
        )
        ...
        if test_dict["function"] == "convert":
            ...
            try:
                parameters.parameters(test_dict["config_file"], numofproc=numofproc)
                test_outcomes[test] = "Passed"
            except Exception as e:
            ...
            raise ValueError(
                "Unknown function variable: " "{0}".format(test_dict["function"])
            )
        ...
        print("".center(100, "-"))
        print(
            "Done With Test #{0} of {1}: {2}".format(i + 1, num_tests, test).center(100)
        )
