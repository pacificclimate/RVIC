...
"""
    if not filename:
        filename = os.path.join(os.path.dirname(__file__), "rvic", "version.py")
...
if ISRELEASED:
    ...
    if pipe is None or pipe.returncode != 0:
        ...
        if os.path.exists("rvic/version.py"):
            warnings.warn(
                "WARNING: Couldn't get git revision, using existing " "rvic/version.py"
            )
            write_version = False
        else:
            warnings.warn(
...
setup(
    ...
    install_requires=["scipy", "numpy < 1.20", "netCDF4", "matplotlib", "pandas < 1.5"],
    ...
    scripts=["scripts/rvic", "tools/find_pour_points.py", "tools/fraction2domain.bash"],
    ext_modules=[
        Extension("rvic_convolution", sources=["rvic/clib/rvic_convolution.c"])
    ],
)
# -------------------------------------------------------------------- #
